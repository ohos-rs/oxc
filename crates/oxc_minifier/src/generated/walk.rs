...
unsafe fn walk_expression<'a, Tr: Traverse<'a>>(
    ...
    match &mut *node {
        ...
        Expression::ArkUIComponentExpression(node) => {
            walk_ark_ui_component_expression(traverser, (&mut **node) as *mut _, ctx)
        }
...
unsafe fn walk_leading_dot_expression<'a, Tr: Traverse<'a>>(
    traverser: &mut Tr,
...
unsafe fn walk_import_meta<'a, Tr: Traverse<'a>>(
    traverser: &mut Tr,
    node: *mut ImportMeta,
    ctx: &mut TraverseCtx<'a>,
) {
...
unsafe fn walk_new_target<'a, Tr: Traverse<'a>>(
    traverser: &mut Tr,
    node: *mut NewTarget,
    ctx: &mut TraverseCtx<'a>,
) {
...
unsafe fn walk_function<'a, Tr: Traverse<'a>>(
    ...
    for item in &mut *((node as *mut u8).add(ancestor::OFFSET_FUNCTION_DECORATORS)
...
unsafe fn walk_lazy_import_declaration<'a, Tr: Traverse<'a>>(
    traverser: &mut Tr,
...
unsafe fn walk_struct_statement<'a, Tr: Traverse<'a>>(
    traverser: &mut Tr,
    ...
    walk_struct_body(
        ...
        (&mut **((node as *mut u8).add(ancestor::OFFSET_STRUCT_STATEMENT_BODY)
            as *mut ArenaBox<StructBody>)) as *mut _,
        ctx,
    );
    ctx.pop_stack(pop_token);
    ctx.set_current_scope_id(previous_scope_id);
...
unsafe fn walk_struct_body<'a, Tr: Traverse<'a>>(
    traverser: &mut Tr,
    ...
    for item in &mut *((node as *mut u8).add(ancestor::OFFSET_STRUCT_BODY_BODY)
        as *mut ArenaVec<StructElement>)
    {
...
unsafe fn walk_struct_element<'a, Tr: Traverse<'a>>(
    traverser: &mut Tr,
...
unsafe fn walk_ark_ui_component_expression<'a, Tr: Traverse<'a>>(
    traverser: &mut Tr,
    node: *mut ArkUIComponentExpression<'a>,
    ctx: &mut TraverseCtx<'a>,
) {
    traverser.enter_ark_ui_component_expression(&mut *node, ctx);
    ...
    walk_expression(
        traverser,
        (node as *mut u8).add(ancestor::OFFSET_ARK_UI_COMPONENT_EXPRESSION_CALLEE)
            as *mut Expression,
        ctx,
    );
    if let Some(field) = &mut *((node as *mut u8)
        .add(ancestor::OFFSET_ARK_UI_COMPONENT_EXPRESSION_TYPE_ARGUMENTS)
        as *mut Option<ArenaBox<TSTypeParameterInstantiation>>)
    {
    ...
    for item in &mut *((node as *mut u8).add(ancestor::OFFSET_ARK_UI_COMPONENT_EXPRESSION_ARGUMENTS)
        as *mut ArenaVec<Argument>)
    {
        walk_argument(traverser, item as *mut _, ctx);
    }
    ctx.retag_stack(AncestorType::ArkUIComponentExpressionChildren);
    for item in &mut *((node as *mut u8).add(ancestor::OFFSET_ARK_UI_COMPONENT_EXPRESSION_CHILDREN)
        as *mut ArenaVec<ArkUIChild>)
    {
        walk_ark_ui_child(traverser, item as *mut _, ctx);
    }
    ctx.retag_stack(AncestorType::ArkUIComponentExpressionChainExpressions);
    for item in &mut *((node as *mut u8)
        .add(ancestor::OFFSET_ARK_UI_COMPONENT_EXPRESSION_CHAIN_EXPRESSIONS)
        as *mut ArenaVec<CallExpression>)
    {
        walk_call_expression(traverser, item as *mut _, ctx);
    }
    ctx.pop_stack(pop_token);
    traverser.exit_ark_ui_component_expression(&mut *node, ctx);
}

unsafe fn walk_ark_ui_child<'a, Tr: Traverse<'a>>(
    traverser: &mut Tr,
    node: *mut ArkUIChild<'a>,
    ctx: &mut TraverseCtx<'a>,
) {
    traverser.enter_ark_ui_child(&mut *node, ctx);
    match &mut *node {
    ...
    traverser.exit_ark_ui_child(&mut *node, ctx);
}

unsafe fn walk_annotation_declaration<'a, Tr: Traverse<'a>>(
    traverser: &mut Tr,
    ...
    walk_annotation_body(
        ...
        (&mut **((node as *mut u8).add(ancestor::OFFSET_ANNOTATION_DECLARATION_BODY)
            as *mut ArenaBox<AnnotationBody>)) as *mut _,
        ctx,
    );
    ctx.pop_stack(pop_token);
    ctx.set_current_scope_id(previous_scope_id);
...
unsafe fn walk_annotation_body<'a, Tr: Traverse<'a>>(
    traverser: &mut Tr,
    ...
    for item in &mut *((node as *mut u8).add(ancestor::OFFSET_ANNOTATION_BODY_BODY)
        as *mut ArenaVec<AnnotationElement>)
    {
...
unsafe fn walk_annotation_element<'a, Tr: Traverse<'a>>(
    traverser: &mut Tr,
