...
pub enum AstType {
    ...
    AssignmentTargetRest = 31,
    AssignmentTargetWithDefault = 32,
    AssignmentTargetPropertyIdentifier = 33,
    AssignmentTargetPropertyProperty = 34,
...
pub enum AstKind<'a> {
    ...
    ImportMeta(&'a ImportMeta) = AstType::ImportMeta as u8,
    NewTarget(&'a NewTarget) = AstType::NewTarget as u8,
    ...
    LazyImportDeclaration(&'a LazyImportDeclaration<'a>) = AstType::LazyImportDeclaration as u8,
...
impl GetSpan for AstKind<'_> {
    ...
    fn span(&self) -> Span {
        match self {
            ...
            Self::ImportMeta(it) => it.span(),
            Self::NewTarget(it) => it.span(),
            ...
            Self::LazyImportDeclaration(it) => it.span(),
...
impl GetAddress for AstKind<'_> {
    ...
    fn address(&self) -> Address {
        match *self {
            ...
            Self::ImportMeta(it) => it.unstable_address(),
            Self::NewTarget(it) => it.unstable_address(),
            ...
            Self::LazyImportDeclaration(it) => it.unstable_address(),
...
impl<'a> AstKind<'a> {
    ...
    #[inline]
    pub fn as_import_meta(self) -> Option<&'a ImportMeta> {
        if let Self::ImportMeta(v) = self { Some(v) } else { None }
    }

    #[inline]
    pub fn as_new_target(self) -> Option<&'a NewTarget> {
        if let Self::NewTarget(v) = self { Some(v) } else { None }
    }
    ...
    #[inline]
    pub fn as_lazy_import_declaration(self) -> Option<&'a LazyImportDeclaration<'a>> {
        if let Self::LazyImportDeclaration(v) = self { Some(v) } else { None }
    }
